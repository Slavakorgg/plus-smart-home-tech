...
public class PaymentStateSuccessPoller extends AbstractPoller<Payment> {
    ...
    protected Optional<Payment> getEntity() {
        return paymentRepository.findFirstByPaymentStateOrderByTouchedAtAsc(PaymentState.SUCCESS);
    }

    @Override
    protected void handleNormally(Payment payment) {
        orderClient.successfulPaymentForOrderId(payment.getOrderId());
